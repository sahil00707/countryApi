...
var all_borders = document.querySelector(".all_borders");
...
async function showHide(x) {
    ...
    if (country_capital.innerHTML == "undefined") {  //hide if country capital is not there
    ...
    if (country_sub_region.innerHTML == "undefined") {   //hide if country subregion is not there
    ...
    var all_border_json=res[x].borders;
    while(all_borders.hasChildNodes()){
        all_borders.removeChild(all_borders.firstChild);
    }
    for(var i=0;i<all_border_json.length;i++){
        var borderNode=document.createElement("DIV");
        var borderTextNode=document.createTextNode(all_border_json[i]);
        borderNode.appendChild(borderTextNode);
        borderNode.classList.add("border");
        all_borders.appendChild(borderNode);
    }
}
